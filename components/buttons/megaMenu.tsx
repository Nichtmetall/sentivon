                    ...
                    <Transition as={Fragment} enter="transition ease duration-500" enterFrom="opacity-0 translate-y-2" enterTo="opacity-100 translate-y-0" leave="transition ease duration-200" leaveFrom="opacity-100 translate-y-0" leaveTo="opacity-0 translate-y-1" show={isOpen}>
                        <div onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)}>
                            <Popover.Panel className="absolute pt-5 px-2 z-10 left-1/2 -translate-x-1/2 transform w-screen max-w-md lg:max-w-3xl">
                                <div className="overflow-hidden rounded-md ring-1 ring-black ring-opacity-5 shadow-lg">
                                    <div className={classNames(props.hasFooter ? 'rounded-t-md' : 'rounded-md', "relative grid gap-6 bg-white dark:bg-gray-800 px-5 py-6 sm:gap-8 sm:p-8 lg:grid-cols-2")}>
                                        {props.items.map((item: any) => (
                                            <Link key={item.title} href={item.href} className="-m-3 p-3 flex showicon rounded-md hover:translate-x-1 hover:bg-gray-100 items-start dark:hover:bg-gray-700 transition">
                                                <div className="flex-shrink-0 rounded-md flex items-center justify-center h-10 w-10 bg-primary-500 text-white sm:h-12 sm:w-12">
                                    ...
                                    {props.hasFooter &&
                                        <div className="p-5 rounded-b-md bg-gray-100 dark:bg-gray-700 sm:p-8">
                                            <Link href="#" className="-m-3 p-3 rounded-md showicon flex hover:translate-x-1 hover:bg-gray-200 dark:hover:bg-gray-600 transition">
